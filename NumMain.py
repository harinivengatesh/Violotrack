    ...
    if ret == True:
        ...
        if possible_plates is not None:
            for i, p in enumerate(possible_plates):
                ...
                conn = mysql.connector.connect(user='root', password='', host='localhost', database='1numberfacedb')
                cursor = conn.cursor()
                ...
                if data is None:
                    ...
                    if data is None:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "insert into entrytb values('','" + str(vno) + "','','" + str(date) + "','" + str(
                                timeStamp) + "')")
                        conn.commit()
                        conn.close()
